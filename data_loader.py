...
def board_totals(data, board=None, year=None) -> dict:
    ...
    # Auto-flag a gender-split gap the workbook itself doesn't already explain.
    # A few boards' gazettes don't attribute every appeared candidate to a
    # gender-tagged group (e.g. late/absent categories); where the source's
    # own Notes column already documents this (Gujranwala, Peshawar), we
    # don't duplicate it — we only add this when there's a real gap AND no
    # existing note already covers it.
    gender_sum = male_app + female_app
    gap = appeared - gender_sum
    if not notes and appeared > 0 and gender_sum > 0 and gap != 0:
        notes.append(
            f"Gender split (Male {male_app:,} + Female {female_app:,} = {gender_sum:,}) is "
            f"{abs(gap):,} {'short of' if gap > 0 else 'more than'} the gazette's Total Appeared "
            f"({appeared:,}); the source workbook doesn't attribute this remainder to a "
            f"gender-tagged group. [Auto-flagged by the dashboard, not printed in the source gazette.]"
        )
