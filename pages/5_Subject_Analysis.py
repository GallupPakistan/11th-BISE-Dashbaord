"""
Subject-wise Analysis — pass % by subject, combined across all boards and per-board.
"""
import pandas as pd
...
from common import (
    ...
    kpi_card, show_chart, subject_pass_hbar,
...
for name in boards_sel:
    df = subject_df_for(data, name, year)
    if df.empty:
        continue
    df = df.copy()
    df["Board"] = name
...
with c1:
    st.markdown(kpi_card("SUBJECTS (RELIABLE)", str(len(reliable)), f"{len(combined)} total · {len(reporting)} boards report subjects", NAVY), unsafe_allow_html=True)
