"""
common.py
Shared design tokens, CSS, chart factories, and small analytics helpers
used by every page of the BISE 11th Class (HSSC Part-I) Results Dashboard.

Visual language mirrors the companion 10th-grade (SSC) dashboard so the two
feel like one product family — same navy/teal palette, same card system,
same chart styling — but every chart-producing function here takes plain
DataFrames/values, so nothing is tied to how the 10th dashboard's raw
sheets were parsed.
"""
import re
...
# ── Hero banner + sidebar chrome ────────────────────────────────────────────
def render_hero_banner(board_count=15):
    watermark = """
      ...
      <div class="hero-inner">
        <div class="hero-left">
          <div class="hero-brand">
            ...
            <div><div class="hero-brand-title">BISE Analytics</div><div class="hero-brand-sub">HSSC Part-I Dashboard</div></div>
          </div>
          <div class="hero-crumb">🏠 / Dashboard / 11th Class Results</div>
          <div class="hero-title-row"><span class="hero-accent"></span>
            <h1 class="hero-title">11th Grade (HSSC Part-I) Results 2024–25</h1></div>
          <p class="hero-desc">Board-wise pass/fail, gender ratios, groups (Pre-Medical / Pre-Engineering / Humanities / Commerce / General Science), subjects, districts &amp; year-over-year trends for all BISE boards.</p>
          <div class="hero-pill">📅 HSSC Part-I · 2024–2025</div>
        </div>
        <div class="hero-stat">
...
def inject_css():
    st.markdown(
        ...
        [data-testid="stToolbarActions"], [data-testid="stAppDeployButton"], .stAppDeployButton {{
        ...
        [data-testid="stSidebarCollapsedControl"] {{
            ...
            color: {SIDEBAR_DARK} !important; z-index: 999999 !important;
        ...
        [data-testid="stSidebar"] .stMultiSelect [data-baseweb="tag"] {{
            background: {NAVY} !important; border: 1px solid {NAVY} !important; color: #FFFFFF !important;
        }}
        [data-testid="stSidebar"] .stMultiSelect [data-baseweb="tag"] span {{ color: #FFFFFF !important; }}
        [data-testid="stSidebar"] .stMultiSelect input {{ color: #1A1A2E !important; }}
        ...
        .kpi-grid {{ display: flex; flex-wrap: wrap; gap: 14px; margin-bottom: 6px; }}
        .kpi-grid .kpi-box {{ flex: 1 1 168px; min-width: 168px; max-width: 240px; }}
        ...
        .hero-title {{ color: #FFFFFF !important; font-size: 1.7rem !important; font-weight: 700 !important;
            margin: 0 !important; line-height: 1.25; }}
        .hero-desc {{ color: rgba(255,255,255,0.92) !important; font-size: 14px; margin: 0 0 14px 0; max-width: 640px; }}
        .hero-pill {{
...
def render_notes(notes):
    """Surface the workbook's own data-quality caveats (Notes column) —
    never invented, always straight from the source sheet."""
    if not notes:
        return
...
def render_sidebar_brand():
    st.markdown(
        ...
        <div>
          ...
          <div class="sidebar-brand-sub">HSSC Part-I Dashboard</div>
        </div>
        </div>""",
        unsafe_allow_html=True,
    )
